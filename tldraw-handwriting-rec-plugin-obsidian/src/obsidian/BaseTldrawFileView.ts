...
export abstract class BaseTldrawFileView<View extends FileView = FileView> {
	...
	#performanceDisplayEl?: HTMLElement
	...
	/**
	 * Initialize and manage the performance metrics display in the header
	 */
	private initializePerformanceDisplay(): void {
		// Recreate the element on each view load to avoid stale nodes.
		this.#performanceDisplayEl?.remove()
		const ownerDocument = this.fileView.contentEl.ownerDocument

		// Create display element
		this.#performanceDisplayEl = ownerDocument.createElement('div')
		this.#performanceDisplayEl.style.cssText = `
			font-size: 11px;
			line-height: 1.2;
			...
			margin-left: 8px;
		`
		this.#performanceDisplayEl.textContent = 'FPS: -- (ms: --.-)'

		const leaf = this.fileView.containerEl.closest('.workspace-leaf')
		const titleContainer = leaf?.querySelector(
			'.view-header-title-container.mod-at-start.mod-fade.mod-at-end, .view-header-title-container'
		)

		if (titleContainer instanceof HTMLElement) {
			titleContainer.appendChild(this.#performanceDisplayEl)
		} else {
			const fallbackHeader = leaf?.querySelector('.view-header')
			if (fallbackHeader instanceof HTMLElement) {
				fallbackHeader.appendChild(this.#performanceDisplayEl)
			} else {
				// Last fallback: attach in the view root so the metric is still visible.
				this.fileView.contentEl.prepend(this.#performanceDisplayEl)
			}
		}

		// Start update loop
		this.startPerformanceDisplayUpdates()

		// Ensure node belongs to the right document in pop-out windows.
		if (this.#performanceDisplayEl.ownerDocument !== ownerDocument) {
			ownerDocument.adoptNode(this.#performanceDisplayEl)
		}
	}

	/**
	 * Start updating performance metrics display
	 */
	private startPerformanceDisplayUpdates(): void {
		if (this.#performanceUpdateInterval) {
			clearInterval(this.#performanceUpdateInterval)
		}

		this.#performanceUpdateInterval = window.setInterval(() => {
			if (!this.#performanceDisplayEl) return

			const metrics = performanceMonitor.getRollingMetrics()
			if (metrics.currentFps <= 0 || metrics.avgFrameTime <= 0) {
				this.#performanceDisplayEl.textContent = 'FPS: -- (ms: --.-)'
				return
			}

			const fps = metrics.currentFps.toFixed(0)
			const frameTimeMs = metrics.avgFrameTime.toFixed(1)
			this.#performanceDisplayEl.textContent = `FPS: ${fps} (ms: ${frameTimeMs})`
		}, 200)
	}
	...
	onload(): void {
		...
		this.initializePerformanceDisplay()
	}
	...
	onunload(): void {
		...
		this.#performanceDisplayEl?.remove()
		this.#performanceDisplayEl = undefined
